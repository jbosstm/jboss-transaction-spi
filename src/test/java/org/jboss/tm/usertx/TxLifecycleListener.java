...
public class TxLifecycleListener implements Synchronization, TransactionLifecycleListener<AssociationLifecycleEventType> {
    ...
    private TransactionLifecycleListenerRegistry registry;
    private volatile int registrationCount = 2;
    private boolean hasEvents = false;

    TxLifecycleListener(TransactionLifecycleListenerRegistry registry) {
    ...
    @Override
    public void onTransactionEvent(TransactionLifecycleEvent transactionEvent) {
        hasEvents = true;

        if (transactionEvent.getReason() == AssociationLifecycleEventType.ASSOCIATED) {
            if (registry != null && registrationCount > 0) {
                ...
                try {
                    registry.addListener(transactionEvent.getTransaction(), listener, EnumSet.<AssociationLifecycleEventType>allOf(AssociationLifecycleEventType.class));
                } catch (TransactionLifecycleTypeNotSupported e) {
        ...
        if (transactionEvent.getReason() == AssociationLifecycleEventType.DISASSOCIATING) {
            assoc--;
            assert (assoc == 0);
            if (acCalled == 1) {
    ...
    private static class L2 implements TransactionLifecycleListener {
        @Override
        public void onTransactionEvent(TransactionLifecycleEvent transactionEvent) {
        }
    }
